#!/usr/bin/env python3
"""
Enhanced API Analyzer v2 with improved parameter extraction from handler functions.
This version properly analyzes function signatures to extract path, query, body, and header parameters.
"""
...
class EnhancedAPIAnalyzerV2:
    """Enhanced API analyzer with improved parameter extraction from handler functions"""
    ...
    def _analyze_python_file(self, file_path: str, content: str) -> List[EnhancedEndpoint]:
        """Analyze Python file using AST with enhanced parameter extraction"""
        try:
    ...
    def _analyze_python_function(self, func_node: ast.FunctionDef, file_path: str, pydantic_models: Dict[str, Any], router_names: set) -> Optional[EnhancedEndpoint]:
        """Analyze a Python function for API endpoint information with enhanced parameter extraction"""
        ...
        parameters = self._analyze_python_parameters_enhanced(func_node, route_info, path_params, pydantic_models)
    ...
    def _analyze_python_parameters_enhanced(self, func_node: ast.FunctionDef, route_info: Dict[str, Any], path_params: List[str], pydantic_models: Dict[str, Any]) -> List[ParameterInfo]:
    ...
    def _determine_parameter_source_enhanced(self, param_name: str, func_node: ast.FunctionDef, route_info: Dict[str, Any], path_params: List[str]) -> ParameterSource:
        ...
        # Check type hints for common patterns (fallback)
        ...
        # Check if it's a Pydantic model (typically body parameter)
        if param_type and any(model in param_type for model in ['BaseModel', 'Model']):
            return ParameterSource.BODY
